  ...
  return (
    <nav className="fixed top-0 left-0 w-screen flex justify-between items-center p-4 bg-transparent z-50 overflow-hidden">
      <div className="flex items-center">
        <img src={companyLogo} alt="Company Logo" className="w-12 h-12 mr-2" /> {/* Replace icon with image */}
      ...
      <div
        className="fixed top-0 left-0 w-full h-full bg-green-200 transform -translate-x-full z-40 menu-container overflow-hidden"
      >
        <div className="flex flex-col justify-center items-center h-full space-y-8">
          {moreList.map((item, index) => (
            <NavLink
              to={item.path}
              key={index}
              className="text-white text-3xl md:text-7xl font-bold flex items-center transition-colors duration-300 hover:text-green-300 common-animation"
              ref={(el) => (commonRef.current[index] = el)}
            >
              <span className="text-white text-base md:text-xl mr-4 flex h-full items-start">{`0${index + 1}`}</span>
              <span>{item.name}</span>
            </NavLink>
          ))}
        </div>
